...
class Cargos extends DbConnectionSQLServer
{
    ...
    protected function obtenerCargosYTrabajadores(/*$c_dir,*/$c_depto)
    {
        $result = array();
        $conn = $this->connect();
    ...
    protected function obtenerCargos($c_depto)
    {
        $result = array();
        $conn = $this->connect();
        $sql = "SELECT ocupacion, ocupaciones.C_OCUP
                    FROM personal
                    JOIN DEPARTAMENTOS ON personal.C_DEPTO = departamentos.C_DEPTO
                    JOIN ocupaciones ON personal.c_ocup = ocupaciones.C_OCUP
                    JOIN DIRECCIONES ON personal.C_DIR = direcciones.C_DIR
                    WHERE ocupaciones.C_OCUP = ? GROUP BY ocupacion, ocupaciones.C_OCUP";
